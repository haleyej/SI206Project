...
def fill_database(unemployment_dict, totals_list, cur, conn):
    ...
    for x in range(len(unemployment_dict.keys())):
        weeks = list(unemployment_dict.keys())

        #creates variables to insert into table
        week_key = weeks[x]
        unemployment_claim_num = unemployment_dict[week_key]
        total = totals_list[x]
        week_num = x + 1
...
def main():
    ...
    fill_database(weekly_umemployment_claims_dict, cumulative_claims_list, cur, conn)
